...
One issue that I had fairly frequently to begin with, and though it's no longer an issue it is an annoyance, is the wording of knitting patterns.
...
Or whatever other pattern file you wish to use.
...
GLOBAL OPTIONS:
   ...
   --no-run, --norun              Prevent the program from running the pattern (default: fals
e)
...
The main things you'll want to define is a row (or a round) and the mix of stitches within this ro (or round).

A row takes a single line, that is to say that no newline (the `enter` key) is used before the end of the row.

Say you have a checker pattern, two squares along the row, you could write:
...
So, the braces (`{}`) group the knits then purls, and the brackets at the end indicate how many times you should repeat this; in this example we have twelve squares, 144 stitches for the row.

### Groups

We've already seen group to be honest, looking back at [Assignments](#assignments) we have a name, followed by an equals sign (`=`) followed by a brace group that spans multiple lines... and that's all a group is... a set of rows.
...
, and this is something you don't use again in the pattern but right here, right now, you need this pattern sixteen times and don't want to copy-paste sixteen times - you could write:
...
So, much like the row, we group these rows and the brackets indicate that we repeat sixteen times.
